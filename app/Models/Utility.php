...
class Utility extends Model
{
    ...
    public static function getValByNameMollie($key)
    {
        $setting = Utility::settings();
        if (!isset($setting[$key]) || empty($setting[$key])) {
            $setting[$key] = '';
    ...
    public static function paymentTaxRate($taxRate, $price, $quantity, $discount = 0)
    {
        if ($taxRate < 10) {
            $underTaxRate = '1.0' . strval($taxRate);
        } else {
            $underTaxRate = '1.' . strval($taxRate);
        }

        return $price - ($price / floatval($underTaxRate));
    }

    public static function recurringpaymentTaxRate($taxRate, $price, $quantity, $discount = 0)
    {
        if ($taxRate < 10) {
            $underTaxRate = '1.0' . strval($taxRate);
        } else {
                ...
                if ($is_active->is_active == 1) {
                    ...
                    if (!empty($content->content)) {
                        ...
                        try {
                            ...
                            // $mailll = Mail::to('alitaimur4@gmail.com')->send(new CommonEmailTemplate($content, $settings));
                        } catch (\Exception $e) {
							\Log::info($e);
