...
class TaxController extends Controller
{
    ...
    {
        //  dd(\Auth::user()->can('delete constant tax'));
        ...
        if(\Auth::user()->can('delete constant tax'))
        {
            if($tax->created_by == \Auth::user()->creatorId())
            {
                ...
                $paymentData  = Payment::whereRaw("find_in_set('$tax->id',tax_id)")->first();
                $recurringpaymentData  = RecurringPayment::whereRaw("find_in_set('$tax->id',tax_id)")->first();

                if(!empty($proposalData) || !empty($billData) || !empty($invoiceData) || !empty($recurringpaymentData) ||  !empty($paymentData))
                {
