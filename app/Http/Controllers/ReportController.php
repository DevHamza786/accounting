...
class ReportController extends Controller
{
    ...
    public function expenseSummary(Request $request)
    {
        if(\Auth::user()->can('expense report'))
        {
            ...
            for($i = 1; $i <= 12; $i++)
            {
             ...
             //   -----------------------------------------Recurring PAYMENT EXPENSE ------------------------------------------------------------
             $recurring = RecurringPayment::selectRaw('sum(recurring_payments.amount) as amount,MONTH(recurring_date) as month,YEAR(recurring_date) as year,category_id')->leftjoin('product_service_categories', 'recurring_payments.category_id', '=', 'product_service_categories.id')->where('product_service_categories.type', '=', 2);
             $recurring->where('recurring_payments.created_by', '=', \Auth::user()->creatorId());
             $recurring->whereRAW('YEAR(recurring_date) =?', [$year]);
 
             if(!empty($request->category))
             {
                $recurring->where('category_id', '=', $request->category);
                 $cat                = ProductServiceCategory::find($request->category);
                 $filter['category'] = !empty($cat) ? $cat->name : '';
             }
             if(!empty($request->vender))
             {
                $recurring->where('vender_id', '=', $request->vender);
 
                 $vend             = Vender::find($request->vender);
                 $filter['vender'] = !empty($vend) ? $vend->name : '';
             }
 
              $recurring->groupBy('month', 'year', 'category_id');
              $recurring =  $recurring->get();
             $tmpArray = [];
             foreach( $recurring as $recurr)
             {
                 $tmpArray[$recurr->category_id][$recurr->month] = $recurr->amount;
             }
             $array = [];
             foreach($tmpArray as $cat_id => $record)
             {
             ...
             $recurringData = RecurringPayment::selectRaw('sum(recurring_payments.amount) as amount,MONTH(recurring_date) as month,YEAR(recurring_date) as year');
             $recurringData->where('recurring_payments.created_by', '=', \Auth::user()->creatorId());
             $recurringData->whereRAW('YEAR(recurring_date) =?', [$year]);
 
             if(!empty($request->category))
             {
                 $recurringData->where('category_id', '=', $request->category);
             }
             if(!empty($request->vender))
             {
                 $recurringData->where('vender_id', '=', $request->vender);
             }
             $recurringData->groupBy('month', 'year');
             $recurringData = $recurringData->get();
 
             $recurrArr = [];
             foreach($recurringData as $k => $recurrData)
             {
                 $recurrArr[$recurrData->month] = $recurrData->amount;
             }
             for($i = 1; $i <= 12; $i++)
             {
                 $recurrTotal[] = array_key_exists($i, $recurrArr) ? $recurrArr[$i] : 0;
             }
    ...
    public function taxSummary(Request $request)
    {
        if(\Auth::user()->can('tax report'))
        {
            ...
            $all_payments = Payment::selectRaw('*, MONTH(created_at) as month,YEAR(created_at) as year')->where('created_by', \Auth::user()->id)->get();
            // dd($all_payments);
        	$expensePaymentTaxesData = [];
            foreach($all_payments as $payment)
            {
                $paymentTax   = [];
                $paymentTaxes = \Utility::tax($payment->tax_id);
                // dd($paymentTaxes);
                foreach($paymentTaxes as $taxe)
                {
                    $taxDataPrice =\Utility::paymentTaxRate($taxe->rate, $payment->amount, 1);
                    $paymentTax[$taxe->name] = $taxDataPrice;
                    // dd($paymentTax);
                }
                $expensePaymentTaxesData[$payment->month][] = $paymentTax;
            }
        	// dd($expensePaymentTaxesData);
            $payment = [];
            foreach($expensePaymentTaxesData as $month => $paymentTaxx)
            {
                $paymentTaxRecord = [];
                foreach($paymentTaxx as $k => $record)
                {
                    foreach($record as $paymentTaxName => $paymentTaxAmount)
                    {
                        if(array_key_exists($paymentTaxName, $paymentTaxRecord))
                        {
                            $paymentTaxRecord[$paymentTaxName] += $paymentTaxAmount;
                    ...
                    $payment['data'][$month] = $paymentTaxRecord;
                    // dd($payment);
                }
            }
            foreach($payment as $paymentMonth => $paymentTaxData)
            {
                $paymentData = [];
                for($i = 1; $i <= 12; $i++)
                {
                    $paymentData[$i] = array_key_exists($i, $paymentTaxData) ? $paymentTaxData[$i] : 0;
                    // dd($paymentData);
                }
            }
            ...
            if(isset($paymentData) && !empty($paymentData))
            {
                foreach($taxList as $taxArr)
                {
                    foreach($paymentData as $month => $tax)
                    {
                        if($tax != 0)
                        {
                            if(isset($tax[$taxArr->name])){
                            	if(isset($backup_expenses[$taxArr->name][$month])){
                                    $backup_expenses[$taxArr->name][$month] = $tax[$taxArr->name] + $backup_expenses[$taxArr->name][$month];                             
                                } else {
                                	$backup_expenses[$taxArr->name][$month] = $tax[$taxArr->name];
                                    // dd($backup_expenses);
                                }
                            }
                            else
                            {
                                $backup_expenses[$taxArr->name][$month] = 0;
                            }
                        }
                        else
                        {
            ...
            $all_recurringpayments = RecurringPayment::selectRaw('*, MONTH(created_at) as month,YEAR(created_at) as year')->where('created_by', '1')->get();
        	$expenseRecurringPaymentTaxesData = [];
            foreach($all_recurringpayments as $recurringpayment)
            {
                $recurringpaymentTax   = [];
                $recurringpaymentTaxes = \Utility::tax($recurringpayment->tax_id);
                foreach($recurringpaymentTaxes as $taxe)
                {
                    $taxDataPrice         = \Utility::recurringpaymentTaxRate($taxe->rate, $recurringpayment->amount, 1);
               
                    $recurringpaymentTax[$taxe->name] = $taxDataPrice;
                
                }
                $expenseRecurringPaymentTaxesData[$recurringpayment->month][] = $recurringpaymentTax;
            }
        	// dd($expenseRecurringPaymentTaxesData);
            $recurringpayment = [];
            foreach($expenseRecurringPaymentTaxesData as $month => $recurringpaymentTaxx)
            {
                $recurringpaymentTaxRecord = [];
                foreach($recurringpaymentTaxx as $k => $record)
                {
                    foreach($record as $recurringpaymentTaxName => $recurringpaymentTaxAmount)
                    {
                        if(array_key_exists($recurringpaymentTaxName, $recurringpaymentTaxRecord))
                        {
                            $recurringpaymentTaxRecord[$recurringpaymentTaxName] += $recurringpaymentTaxAmount;
                        }
                        else
                        {
                            $recurringpaymentTaxRecord[$recurringpaymentTaxName] = $recurringpaymentTaxAmount;
                    ...
                    $recurringpayment['data'][$month] = $recurringpaymentTaxRecord;
                }
               

            }
            foreach($recurringpayment as $recurringpaymentMonth => $recurringpaymentTaxData)
            {
                $recurringpaymentData = [];
                for($i = 1; $i <= 12; $i++)
                {
                    $recurringpaymentData[$i] = array_key_exists($i, $recurringpaymentTaxData) ? $recurringpaymentTaxData[$i] : 0;
       
                }
            }
            if(isset($recurringpaymentData) && !empty($recurringpaymentData))
            {
                foreach($taxList as $taxArr)
                {
                    $indexer = 0;
                    foreach($recurringpaymentData as $month => $tax)
                    {
                    ...
                    // dd($indexer++);
                        if($tax != 0)
                        {
                            if(isset($tax[$taxArr->name]))
                            {
                                if(isset($expenses[$taxArr->name][$month])){
                                    $expenses[$taxArr->name][$month] = $tax[$taxArr->name] + $expenses[$taxArr->name][$month];
                                } else {
                                	$expenses[$taxArr->name][$month] = $tax[$taxArr->name];
                                }
                            }
            ...
            foreach($backup_expenses as $key => $exp){
                foreach($exp as $index => $monthVise){           
                    if($monthVise != 0){
                        $expenses[$key][$index] = $monthVise;   
                        // dd($expenses);
                    } else {
                    	$expenses[$key][$index] = 0;
                    }
                }
            }
            ...
            $filter['startDateRange'] = 'Jan-' . $year;
            $filter['endDateRange']   = 'Dec-' . $year;
            return view('report.tax_summary', compact('filter', 'data'), $data);
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function profitLossSummary(Request $request)
    {
        ...
        if(\Auth::user()->can('loss & profit report'))
        {
            ...
            $recurring = RecurringPayment::selectRaw('sum(recurring_payments.amount) as amount,MONTH(recurring_date) as month,YEAR(recurring_date) as year,category_id');
            $recurring->where('created_by', '=', \Auth::user()->creatorId());
            $recurring->whereRAW('YEAR(recurring_date) =?', [$year]);
            $recurring->groupBy('month', 'year', 'category_id');
            $recurring = $recurring->get();
            ...
            foreach($recurring as $recurr)
            {
                $tmpExpenseArray[$recurr->category_id][$recurr->month] = $recurr->amount;
            }

            $recurrArray       = [];
            $recurrCatAmount_1 = $recurrCatAmount_2 = $recurrCatAmount_3 = $recurrCatAmount_4 = 0;
            foreach($tmpExpenseArray as $cat_id => $record)
            {
                $tmp             = [];
                $tmp['category'] = !empty(ProductServiceCategory::where('id', '=', $cat_id)->first()) ? ProductServiceCategory::where('id', '=', $cat_id)->first()->name : '';
                ...
                for($i = 1; $i <= 12; $i++)
                {
                    $recurrSumData[] = array_key_exists($i, $record) ? $record[$i] : 0;

                }

                $month_1 = array_slice($recurrSumData, 0, 3);
                $month_2 = array_slice($recurrSumData, 3, 3);
                $month_3 = array_slice($recurrSumData, 6, 3);
                $month_4 = array_slice($recurrSumData, 9, 3);

                $recurrData[__('Jan-Mar')] = $sum_1 = array_sum($month_1);
                $recurrData[__('Apr-Jun')] = $sum_2 = array_sum($month_2);
                $recurrData[__('Jul-Sep')] = $sum_3 = array_sum($month_3);
                $recurrData[__('Oct-Dec')] = $sum_4 = array_sum($month_4);
                $recurrData[__('Total')]   = array_sum(
                    array(
                        $sum_1,
                        $sum_2,
                        $sum_3,
                ...
                $recurrCatAmount_1 += $sum_1;
                $recurrCatAmount_2 += $sum_2;
                $recurrCatAmount_3 += $sum_3;
                $recurrCatAmount_4 += $sum_4;

                $data['month'] = array_keys($recurrData);
                $tmp['amount'] = array_values($recurrData);

                $expenseArray[] = $tmp;

            }

            $data['expenseCatAmount'] = $recurrCatAmount = [
                $recurrCatAmount_1,
                $recurrCatAmount_2,
                $recurrCatAmount_3,
                $recurrCatAmount_4,
